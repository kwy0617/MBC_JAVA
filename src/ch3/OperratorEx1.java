...
public class OperratorEx1 {
	...
	public static void main(String[] args) {
		...
		System.out.println(x % y);			// 10
		// %(mod) 연산의 특징 ... 계산 결과는 우측 피연산자 값의 범위에 있음.
		...
		// 나눗셈 연산에서 예외
		// Inpinity(무한대), NaN(Not a Number)
		// 좌측은 정수이고, 우측 피연산자가 0인 경우. ArithmeticException 발생
		//  5 / 0.0  => 무한대
		//  5 % 0.0  => NaN
		// 이 두 가지의 경우에 연산은 수행하면 안됨.
		// 무한대 혹은 NaN인지 확인하는 에서드...
		// Double,IsInfinit()의 Double.isNaN() 에서드(함수)를 통해서 무한대 혹은 NaN인지
		// 확인이 가능함.
		...
		System.out.println("문자열 비교 -> test 외 test의 비교");
		System.out.println("test".equals("test"));	// 같은지 여부 확인
		System.out.println(!"test".equals("test"));	// 같지 않은지 여부 확인
		
		// 논리 연산자 
		//  논리곱(&&)-and , 논리합(||)-or, 베타적 논리합(^), 논리 부정(ㅣ)
		// 비교 연산자와 같이 흐름제어를 위한 if, 반복문 for, while 에서 주료 사용함.
		...
		System.out.println();
		System.out.println(true || true);		// shirtcut 연산
		System.out.println(true || false);
		...
		// 비트 이동 연산자
		// 비트를 좌측 또는 우측으로 일어서 이동시키는 연산을 수행
		//  <<, >>. >>>
		// <<
